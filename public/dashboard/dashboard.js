...
async function refreshLive(encoded){const id=decodeURIComponent(encoded);const box=$('live-'+id);try{const b=await api('/api/v1/admin/devices/'+encoded+'/live');const live=b.live;const c=live?.currentCall;if(c?.active){box.className='live call-active';const age=live.observedAt?Math.round((Date.now()-new Date(live.observedAt))/1000):null;const liveDuration=c.startedAt?Math.max(Number(c.durationSeconds||0),Math.round((Date.now()-new Date(c.startedAt))/1000)):Number(c.durationSeconds||0);box.innerHTML=`<b>☎ CURRENT CALL</b><br>${safe(c.contactName||'Unknown')} ${c.phoneNumber?`· ${safe(c.phoneNumber)}`:''}<br>${safe(c.type)} · ${safe(c.direction)} · ${fmt(liveDuration)}${age!=null?`<br><span class="muted">device update ${age}s ago</span>`:''}`;}else{box.className='live';box.innerHTML=`<b>No active call reported</b>${live?.observedAt?`<br><span class="muted">last device update ${safe(new Date(live.observedAt).toLocaleString())}</span>`:''}`}}catch(e){box.textContent='Current call unavailable: '+e.message}}
...
function renderSnapshot(result){
  ...
  return [
    ...
    rows('Top app usage',usage,x=>`<div>${safe(x.appName||x.packageName)} — <b>${fmt(x.durationSeconds)}</b></div>`),
...
async function requestLogs(encoded){const id=decodeURIComponent(encoded),status=$('status-'+id),logs=$('logs-'+id);status.textContent='Request queued…';logs.classList.add('hidden');try{const c=await api('/api/v1/admin/devices/'+encoded+'/commands',{method:'POST',body:JSON.stringify({type:'refresh_logs'})});const deadline=Date.now()+30000;while(Date.now()<deadline){await new Promise(r=>setTimeout(r,2000));const result=await api('/api/v1/admin/commands/'+encodeURIComponent(c.commandId));if(result.command.status==='completed'){status.textContent='Fresh device snapshot received ✓';logs.classList.remove('hidden');logs.innerHTML=renderSnapshot(result.command.result);refreshLive(encoded);return;}}status.textContent='Still waiting for device. It will process the request on notification activity, app foreground, or Android periodic background check.';}catch(e){status.textContent=e.message}}

async function requestCurrentCall(encoded){const id=decodeURIComponent(encoded),status=$('status-'+id);status.textContent='Requesting current call…';try{const c=await api('/api/v1/admin/devices/'+encoded+'/commands',{method:'POST',body:JSON.stringify({type:'current_call'})});const deadline=Date.now()+15000;while(Date.now()<deadline){await new Promise(r=>setTimeout(r,1500));const result=await api('/api/v1/admin/commands/'+encodeURIComponent(c.commandId));if(result.command.status==='completed'){status.textContent='Current-call request answered ✓';await refreshLive(encoded);return;}}status.textContent='No immediate response; showing the latest live call state already reported by Android.';await refreshLive(encoded);}catch(e){status.textContent=e.message}}

$('load').onclick=load;
window.requestLogs=requestLogs;window.refreshLive=refreshLive;window.requestCurrentCall=requestCurrentCall;
if(token) load();
setInterval(()=>document.querySelectorAll('[id^="live-"]').forEach(el=>refreshLive(encodeURIComponent(el.id.slice(5)))),10000);
